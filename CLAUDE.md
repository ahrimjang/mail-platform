...
Statuses: campaigns `QUEUED → SENDING → COMPLETED`; messages `PENDING → SENT | FAILED | BOUNCED | SUPPRESSED`. **Message status is delivery-outcome only — engagement (opened/clicked) is event-derived, never a status.** Postgres is the **state store**, not the queue.
