...
대용량 이메일 발송 플랫폼 "Outpace"(Notifuse 참고), **멀티테넌트 SaaS**(가입=워크스페이스, ADMIN/OPERATOR 역할, 발송 인프라는 플랫폼 소유 — 과금은 월 발송량 기준(V20에서 BYO 폐기)). **목표: 2026-09 전 실운영 개시·매출** — 문서·커밋에 "포트폴리오/면접/데모/POC" 류 워딩 금지(서비스 관점으로 서술). 핵심: **API는 큐에 적재 후 즉시 반환, 워커가 비동기로 드레인**. JWT 인증, RabbitMQ 발송 큐(DLQ), Kafka 참여 이벤트 스트림(`mail.events`), SMTP 발송(개발은 MailHog), 예약 발송·취소, 캠페인 기간(수집 컷오프)·임시저장·불러오기, A/B 테스트(해시 분배 + 승자 자동발송), 참여도 세그먼트(리스트 캠페인을 오픈/클릭률로 좁혀 발송 — 팬아웃 시점 평가), 억제/수신거부(전역 + 리스트 단위 옵트아웃), 오픈/클릭 추적(캠페인별 링크 랭킹 + 실행 구간), 대시보드·분석 집계(퍼널/건강도/오픈 히트맵), 수신자 활동 타임라인, 바운스 웹훅(generic + **SES/SNS 어댑터** — 아마존 서명검증·구독 자동승인·SES 파서, 토글 `APP_WEBHOOK_SNS_VERIFY_SIGNATURE`), 테스트 발송·발송 전 확인 모달·빌트인 템플릿 copy-on-write·캠페인 등록자 기록, **템플릿/이메일 계층 분리**(V27 — 템플릿은 재사용 디자인 자산, `emails`가 캠페인에 실제 쓰는 콘텐츠: 템플릿을 복사해 만들고 캠페인 등록 시 스냅샷 — 새 캠페인은 이메일을 선택), 월 발송량 미터링, **테넌트별 발송 속도 제한**(Postgres 토큰버킷 + TTL 파킹 큐 — 관리 페이지에서 설정), **Grafana 메트릭**(Micrometer 어댑터 계측 → Prometheus, 대시보드 파일 프로비저닝), `{{변수}}` 템플릿(블록/텍스트/HTML 에디터 + DB 시딩 기본 템플릿), 트랜잭셔널 발송, 연락처/리스트(CSV + 동의 기록, 페이지드 테이블). 로드맵: [docs/ROADMAP-scale.md](docs/ROADMAP-scale.md) · [docs/TODO-ses-sns.md](docs/TODO-ses-sns.md).
