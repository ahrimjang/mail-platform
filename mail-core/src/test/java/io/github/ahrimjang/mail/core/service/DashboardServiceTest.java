...
class DashboardServiceTest {
    ...
    void stats_fillsEveryDayOfTheWindowEvenWithoutActivity() {
        when(messages.aggregateDailyOutcomes(eq(WS), any(), any())).thenReturn(List.of());
        when(events.aggregateDailyEngagement(eq(WS), any(), any())).thenReturn(List.of());
        when(contacts.countByWorkspace(WS)).thenReturn(3L);
        when(suppressions.countByWorkspace(WS)).thenReturn(1L);
    ...
    void stats_windowBoundaryUsesInstantOfLocalMidnight() {
        ...
        verify(messages).aggregateDailyOutcomes(WS, startOfToday, ZoneId.systemDefault());
        verify(events).aggregateDailyEngagement(WS, startOfToday, ZoneId.systemDefault());
    }
}
