...
**추가(같은 날)**: 후속 배포가 이번엔 "kafka unhealthy"로 실패. 2주째 정상인
브로커였다 — 원인은 헬스체크 자체. `kafka-broker-api-versions.sh` 는 검사마다
JVM 을 새로 띄우는데, 배포 중 api JVM 기동이 CPU 를 포화시키자 이 무거운 검사가
연속 타임아웃돼 멀쩡한 kafka 가 unhealthy 로 찍혔다. 검사를 리스너 포트 TCP
응답(`/dev/tcp`)으로 교체(30초 간격) — 상시 CPU 낭비도 함께 제거. 교훈:
**헬스체크는 검사 대상보다 가벼워야 한다.** JVM 스폰형 검사는 저사양 노드에서
그 자체가 부하이자 오탐원이다.
