...
| **AI 기능 방향** (2026-09-18 추가) | 웹 내장 AI 먼저 · MCP 먼저 · 둘 다 동시 | **웹 내장 AI 먼저**, MCP 는 포탈 요구(포탈 안에서 발송까지 끝내야 하는지) 확인 후 | 웹 내장 AI 는 모든 고객에게 파는 기능이 되고 발송 권한을 AI 에 주지 않아 더 안전. 포탈 안에서 발송까지 끝내야 한다면 MCP 가 필수 — [ai-compose-design.md](ai-compose-design.md) "사용자 모델 연결" |
| **웹 내장 AI 의 모델** | 우리 키로 한 모델 · 사용자가 모델 등록 | **사용자가 등록**(OpenAI 호환 형식부터), 우리는 입력·출력 형식과 검증만 책임 | AI 비용이 사용자에게 가므로 발송량 과금과 충돌이 사라지고 사내 모델도 연결 가능. 대신 키가 없는 사용자는 AI 를 못 쓰고, SSRF·키 보관 대책이 필요 |
| **웹 내장 AI 의 플랜 범위** | 전 플랜 · STANDARD 이상 | 전 플랜(비용이 사용자 몫) | 유료 전환 동기로 쓸지 여부 |
