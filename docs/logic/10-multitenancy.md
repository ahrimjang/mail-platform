...
| | |
|---|---|
| **왜 필요했나** | 여러 회사가 한 서비스를 함께 쓰는 SaaS 라 데이터가 절대 섞이면 안 된다. |
| **무엇을 썼나** | **공유 스키마 + `workspace_id`**(V16) — 최상위 데이터에만 워크스페이스를 달고, 남의 데이터는 404 로 숨긴다. ADMIN/OPERATOR 역할. 예외는 플랫폼 운영자 콘솔 하나(V35). 발송 인프라는 플랫폼 소유 + 발송량 과금(V20). |
| **결과** | 다른 회사 데이터는 존재 자체가 보이지 않는다. 회사별로 스키마를 나누지 않아 운영이 단순하다. |
| **리뷰에서 한 줄로** | 가입하면 회사 공간이 생기고, 모든 데이터는 그 공간 안에서만 보인다. |
