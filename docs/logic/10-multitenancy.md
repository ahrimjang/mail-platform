...
<table>
<tr><th align="left">왜 필요했나</th><td>여러 회사가 한 서비스를 함께 쓰는 SaaS 라 데이터가 절대 섞이면 안 된다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>공유 스키마 + <code>workspace_id</code></b>(V16) — 최상위 데이터에만 워크스페이스를 달고, 남의 데이터는 404 로 숨긴다. ADMIN/OPERATOR 역할. 예외는 플랫폼 운영자 콘솔 하나(V35). 발송 인프라는 플랫폼 소유 + 발송량 과금(V20).</td></tr>
<tr><th align="left">결과</th><td>다른 회사 데이터는 존재 자체가 보이지 않는다. 회사별로 스키마를 나누지 않아 운영이 단순하다.</td></tr>
<tr><td colspan="2">가입하면 회사 공간이 생기고, 모든 데이터는 그 공간 안에서만 보인다.</td></tr>
</table>
...
전환의 동기는 기술이 아니라 과금 모델이다. 처음(V16)에는 비용이 큰 인프라(발송 SMTP/SES,
파일 저장소)를 고객 소유 계정에 연결(BYO)해 비용이 고객에게 직접 청구되는 그림이었으나,
**2026-07-20(V20)에 플랫폼 소유 인프라 + 월 발송량 기준 과금으로 전환**했다 — 온보딩 마찰과
발신 평판 통제 문제, 그리고 미터링·rate limit이 갖춰져 사용량 과금의 전제가 완성됐기 때문.
관리 페이지의 프로바이더 선택은 그때 제거됐고, "사용량 · 요금" 카드(월 발송량 = 청구 기준)가
그 자리를 잇는다. 번복 경위 전체: [../ROADMAP-scale.md](../ROADMAP-scale.md) 설계 노트.
