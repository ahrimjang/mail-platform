...
| | |
|---|---|
| **왜 필요했나** | 어떤 제목·본문이 더 잘 열리는지 감이 아니라 데이터로 정하고 싶다. |
| **무엇을 썼나** | 이메일 **SHA-256 해시로 A/B 결정적 분배**, 일부만 먼저 보내고 나머지는 보류(홀드아웃). 30초 주기 스케줄러가 평가하고, 승자 확정은 조건부 UPDATE 로 한 번만. 최소 표본·동률 유예·24시간 상한 규칙. |
| **결과** | 재시도해도 같은 수신자는 늘 같은 안을 받는다. 표본이 부족하면 성급하게 승자를 정하지 않는다. |
| **리뷰에서 한 줄로** | 일부에게 두 안을 보내 보고, 이긴 안을 나머지에게 자동으로 보낸다. |
