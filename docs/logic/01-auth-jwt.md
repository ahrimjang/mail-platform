...
| | |
|---|---|
| **왜 필요했나** | API 를 아무나 호출하면 안 되고, 가입만으로 스팸을 보낼 수 있으면 안 된다. api 를 여러 대로 늘려도 로그인 상태가 유지돼야 한다. |
| **무엇을 썼나** | **JWT**(서버가 세션을 저장하지 않는 서명 토큰) + **BCrypt** 비밀번호 해시. 가입 이메일 인증(V25), 구글 로그인(V26), 로그인 실패 잠금, nginx 의 IP당 속도 제한. |
| **결과** | 서버는 토큰 서명만 검증하므로 api 를 늘려도 그대로 동작한다. 이메일 인증 전에는 발송 경로가 잠긴다. |
| **리뷰에서 한 줄로** | 로그인은 세션 없는 JWT, 가입 남용은 이메일 인증으로 막는다. |
