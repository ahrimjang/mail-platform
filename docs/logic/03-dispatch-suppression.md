...
| | |
|---|---|
| **왜 필요했나** | 큐는 "최소 한 번" 전달이라 같은 메시지가 두 번 올 수 있다. 수신거부·반송 주소로 보내면 평판이 떨어지고 법 위반이다. |
| **무엇을 썼나** | **조건부 UPDATE claim**(PENDING→SENDING 을 한 문장으로 선점) + claim 토큰, 발송 전 억제 목록 확인, 리스너 동시성 8, 수신거부 푸터와 원클릭 수신거부 헤더, SMTP 타임아웃. |
| **결과** | 처리량 **20.5 → 157.7 msg/s**(약 7.7배), 동시 소비자 12개에서 **이중발송 0**. 로컬 측정 — [RETRO-scaling](../RETRO-scaling.md). |
| **리뷰에서 한 줄로** | 한 통은 먼저 선점한 워커만 보내고, 수신거부한 주소에는 절대 보내지 않는다. |
