...
| | |
|---|---|
| **왜 필요했나** | 성과를 판단하려면 열람·클릭을 알아야 한다. 매번 이벤트 전체를 세면 데이터가 쌓일수록 목록 화면이 느려진다. |
| **무엇을 썼나** | 1×1 **오픈 픽셀**, 서명된 **클릭 리다이렉트**. 추적 요청은 **Kafka**(`mail.events`)에 이벤트만 넣고, 워커가 읽어 DB 에 반영한다. V36 에서 캠페인별 카운터와 완료 캠페인 스냅샷을 저장한다. |
| **결과** | 추적 응답은 DB 쓰기 없이 빠르게 끝난다. 캠페인 목록은 이벤트 수와 무관하게 저장된 값을 읽는다. Kafka 를 처음부터 다시 읽어도 카운트 불일치 0(로컬 검증). |
| **리뷰에서 한 줄로** | 열람·클릭은 이벤트 스트림으로 모으고, 화면은 미리 세어 둔 숫자를 읽는다. |
