...
- **프로바이더 한도**: 실제 릴레이(SES 등)는 초당 발송 한도가 있습니다. BYO 구조에서
  자기 계정의 한도를 아는 건 그 회사이므로, **관리 페이지에서 회사가 스스로 자기 속도를
  설정**합니다(`send_rate_per_sec`, 비우면 무제한).
...
(`types.ts` 미러), 관리 페이지의 "인프라 설정(BYO)" 카드에 입력을 붙였습니다. 검증은
`WorkspaceService.update`에서 null 또는 ≥1. BYO 스토리와 맞물립니다 — "당신이 연결한
SES의 초당 한도를 여기 적어 두면, 대량 캠페인도 그 속도로 나눠 나갑니다".
