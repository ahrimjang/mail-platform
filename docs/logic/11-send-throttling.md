...
| | |
|---|---|
| **왜 필요했나** | 한 회사가 대량 발송을 걸면 다른 회사 메일이 그 뒤에 줄을 선다(noisy neighbor). SES 에는 초당 발송 한도도 있다. |
| **무엇을 썼나** | **Postgres 토큰 버킷**(채우기와 꺼내기를 UPDATE 한 문장으로) + 한도를 넘은 메시지를 1초 쉬게 하는 **TTL 파킹 큐**. 플랜별 상한. 토큰 확인은 claim 보다 먼저. |
| **결과** | 초당 3통 설정 시 실제 **정확히 3.0 msg/s**, 그 와중에 제한 없는 회사의 10통은 **0.3초**에 끝났다. Redis 없이 구현. 로컬 측정 — [RETRO-scaling](../RETRO-scaling.md). |
| **리뷰에서 한 줄로** | 회사별로 초당 발송량을 제한해, 큰 고객이 작은 고객을 막지 않게 한다. |
