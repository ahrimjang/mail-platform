...
<table>
<tr><th align="left">왜 필요했나</th><td>한 회사가 대량 발송을 걸면 다른 회사 메일이 그 뒤에 줄을 선다(noisy neighbor). SES 에는 초당 발송 한도도 있다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>Postgres 토큰 버킷</b>(채우기와 꺼내기를 UPDATE 한 문장으로) + 한도를 넘은 메시지를 1초 쉬게 하는 <b>TTL 파킹 큐</b>. 플랜별 상한. 토큰 확인은 claim 보다 먼저.</td></tr>
<tr><th align="left">결과</th><td>초당 3통 설정 시 실제 <b>정확히 3.0 msg/s</b>, 그 와중에 제한 없는 회사의 10통은 <b>0.3초</b>에 끝났다. Redis 없이 구현. 로컬 측정 — <a href="../RETRO-scaling.md">RETRO-scaling</a>.</td></tr>
<tr><td colspan="2">회사별로 초당 발송량을 제한해, 큰 고객이 작은 고객을 막지 않게 한다.</td></tr>
</table>
...
- **Redis가 아니라 Postgres인 이유.** ROADMAP 초안은 "공유 상태는 Redis"였지만, 이
  ...
  정합성이 보장되고, 운영 대상·장애 지점이 하나 줄어듭니다. 비용은 스로틀된 테넌트의
