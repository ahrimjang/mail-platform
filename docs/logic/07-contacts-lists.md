...
| | |
|---|---|
| **왜 필요했나** | 수신자 주소를 매번 요청에 나열할 수 없다. 수신자가 해지했는데 CSV 를 다시 올리면서 되살아나면 안 된다. |
| **무엇을 썼나** | **연락처**(이메일·이름·자유 속성) + **리스트** + 멤버십 테이블. CSV 가져오기(거부 사유·오타 교정 후보 반환). 해지 기록은 전역 억제와 리스트 단위 해지로 **멤버십과 따로** 보관한다. 참여도(최근 180일 오픈·클릭률) 세그먼트. |
| **결과** | `listId` 하나로 발송 대상을 지정한다. 해지 결정은 CSV 재가져오기에도 뒤집히지 않는다. |
| **리뷰에서 한 줄로** | 수신자는 리스트로 묶어 관리하고, 해지 기록은 명단과 분리해 지킨다. |
