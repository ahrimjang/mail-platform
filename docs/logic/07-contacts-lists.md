...
<table>
<tr><th align="left">왜 필요했나</th><td>수신자 주소를 매번 요청에 나열할 수 없다. 수신자가 해지했는데 CSV 를 다시 올리면서 되살아나면 안 된다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>연락처</b>(이메일·이름·자유 속성) + <b>리스트</b> + 멤버십 테이블. CSV 가져오기(거부 사유·오타 교정 후보 반환). 해지 기록은 전역 억제와 리스트 단위 해지로 <b>멤버십과 따로</b> 보관한다. 참여도(최근 180일 오픈·클릭률) 세그먼트.</td></tr>
<tr><th align="left">결과</th><td><code>listId</code> 하나로 발송 대상을 지정한다. 해지 결정은 CSV 재가져오기에도 뒤집히지 않는다.</td></tr>
<tr><td colspan="2">수신자는 리스트로 묶어 관리하고, 해지 기록은 명단과 분리해 지킨다.</td></tr>
</table>
...
(`{"company":"ACME"}` 등)에 표준 키 4개(email/firstName/lastName/**name**)를 얹습니다. `name` 은 2026-09 에 추가됐습니다 — 에디터 기본 문구와 플레이스홀더가 전부 `{{name}}` 을 권하는데 채워주는 곳이 없어 "안녕하세요 님"으로 나가고 있었습니다. 성+이름(홍길동), 없으면 한쪽만, 그것도 없으면 이메일 아이디 — 어떤 경우에도 빈칸으로 보내지 않습니다(`Contact.displayName`, 직접 입력 수신자도 같은 규칙).
...
한 줄 = `email,firstName,lastName`. **이미 있는 이메일은 skip(멱등)** — 단 `listId`가 주어지면 기존 연락처라도 그 리스트에는 넣어 줍니다. 같은 CSV를 두 번 부어도 중복 연락처가 생기지 않습니다. 2026-09 부터 두 가지가 더 있습니다: ① 첫 줄의 첫 칸이 `@` 없는 흔한 열 이름(`email`, `이메일` 등)이면 **제목 줄로 보고 조용히 건너뜁니다** — 안내문이 권하는 형식을 제목 줄째 붙여넣는 게 보통인데 그 줄이 "거부 1건"으로 잡히면 사용자는 뭘 잘못했는지 모릅니다(첫 줄에만 적용). ② 배달이 불가능한 주소(형식 오류·테스트 도메인·`gmial.com` 같은 오타)는 skip 이 아니라 **rejected** 로 분리해 사유·교정 후보와 함께 돌려줍니다(`EmailAddressValidator`, 평판 가드 1단). 콘솔의 CSV 가져오기에서 리스트 선택은 **선택 사항**입니다(리스트 없이 수신자로만 등록 가능).
