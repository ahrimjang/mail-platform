...
| | |
|---|---|
| **왜 필요했나** | 반송 대부분은 발송하고 한참 뒤에 통보된다. 반송 주소에 계속 보내면 SES 평판이 떨어지고 계정이 정지될 수 있다. |
| **무엇을 썼나** | **웹훅**(일반형 + SES/SNS 어댑터 — 서명 검증, 구독 자동 승인, 토픽 허용목록). 반송·스팸 신고 주소를 억제 목록에 자동 등록하고, 바운스율 10% 를 넘은 워크스페이스는 자동 정지(V32). CloudWatch 경보 3종. |
| **결과** | 2026-09-10 운영에서 SES 반송 시뮬레이터로 실증 — 반송 통보부터 억제 등록까지 사람 손 없이 처리된다. |
| **리뷰에서 한 줄로** | 반송은 SES 가 알려주면 자동으로 차단 목록에 올리고, 나쁜 고객은 자동으로 멈춘다. |
