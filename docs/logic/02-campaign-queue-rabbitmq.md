...
<table>
<tr><th align="left">왜 필요했나</th><td>수신자가 많으면 API 가 직접 보내다 타임아웃이 난다. 초기에는 캠페인 <b>생성</b>조차 수신자 수에 비례해 5천 명에 12.1초가 걸렸다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>RabbitMQ</b> 작업 큐. API 는 캠페인을 저장하고 팬아웃 잡 1건만 넣은 뒤 바로 응답하고, 워커가 수신자를 1,000명씩 나눠 발송 잡으로 펼친다. 중복 팬아웃은 <code>QUEUED→EXPANDING</code> 조건부 UPDATE 로 막는다.</td></tr>
<tr><th align="left">결과</th><td>생성 응답이 1천~10만 명 모두 <b>40~66ms</b>로 일정해졌다(5천 명 기준 약 240배). 로컬 k6 측정 — <a href="../RETRO-scaling.md">RETRO-scaling</a>.</td></tr>
<tr><td colspan="2">API 는 접수만 하고 즉시 돌려주고, 발송은 워커가 큐에서 꺼내 비동기로 한다.</td></tr>
</table>
...
> **2026-09 변경 노트** — 아래 인용 뒤에 세 가지가 더 들어갔습니다. ① 루프 시작 전 `messages.maxContactIdByCampaign` 로 **재개 커서**를 잡아, 스위퍼가 EXPANDING 고착을 되돌린 캠페인은 이미 만든 메시지 뒤부터 잇습니다. ② 페이지마다 캠페인 상태를 보고 **중단(CANCELED)이면 멈춥니다**. ③ 페이지의 억제 주소를 일괄 조회해 **SUPPRESSED 로 바로 기록하고 큐에 넣지 않습니다**(행은 남겨 "발송 제외" 통계 유지). 셋 다 [13 문서](13-recovery-and-abort.md)에 설명이 있습니다.
...
릴리스 시점의 참여도로 거른다는 뜻이고(2026-09 부터 집계는 **해당 워크스페이스·최근 180일**로 한정 — `ContactEngagementService.WINDOW`, 콘솔 미리보기와 같은 창), "누가 대상인가"가 작성 시점에 굳는 스냅샷
...
- **DLQ**: 계속 실패하는 잡(포이즌 메시지)이 큐를 막고 무한 재시도되는 것을 막는 표준 패턴. 2026-09 부터는 워커의 `DeadLetterListener` 가 DLQ 를 소비해 메시지를 FAILED 로 확정하고 캠페인 완료 판정·인앱 알림·지표(`mail.dlq.received`)까지 처리합니다 — "운영자가 DLQ 만 보면 된다"가 아니라 코드가 봅니다([13 문서](13-recovery-and-abort.md) 3-2).
...
- **저장과 발행이 한 트랜잭션이 아닌 한계**: 저장 직후 프로세스가 죽으면 "행은 있는데 잡이 없는" 메시지가 남습니다. 2026-09 부터 워커의 **복구 스위퍼**(60초)가 릴리스됐는데 10분 넘게 PENDING 인 메시지를 재발행하고, 잡 없는 QUEUED 캠페인은 팬아웃을 재발행하거나(메시지가 없으면) CANCELED 로 닫습니다. 재발행은 소비 쪽 claim 이 멱등하게 받으므로 오탐이어도 안전합니다 — outbox 없이 "주기적 재조정"으로 같은 효과를 냅니다([13 문서](13-recovery-and-abort.md) 3-3). 아울러 `create()` 의 대상 검증을 저장 앞으로 옮겨 검증 실패가 고아 행을 남기지 않습니다.
...
- **crash 내성**: claim과 발행 사이에서 워커가 죽으면 그 캠페인의 메시지는 PENDING인 채 남습니다(잡만 없음). `enqueued_at`이 이미 찍혀 스케줄러는 다시 집지 않지만, 2026-09 부터는 **복구 스위퍼**가 10분 뒤 이 행들을 재발행합니다(리스트 캠페인이면 팬아웃 재발행) — 예전의 "수동 복구" 트레이드오프는 사라졌습니다([13 문서](13-recovery-and-abort.md)).
