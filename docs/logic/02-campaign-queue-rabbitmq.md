...
| | |
|---|---|
| **왜 필요했나** | 수신자가 많으면 API 가 직접 보내다 타임아웃이 난다. 초기에는 캠페인 **생성**조차 수신자 수에 비례해 5천 명에 12.1초가 걸렸다. |
| **무엇을 썼나** | **RabbitMQ** 작업 큐. API 는 캠페인을 저장하고 팬아웃 잡 1건만 넣은 뒤 바로 응답하고, 워커가 수신자를 1,000명씩 나눠 발송 잡으로 펼친다. 중복 팬아웃은 `QUEUED→EXPANDING` 조건부 UPDATE 로 막는다. |
| **결과** | 생성 응답이 1천~10만 명 모두 **40~66ms**로 일정해졌다(5천 명 기준 약 240배). 로컬 k6 측정 — [RETRO-scaling](../RETRO-scaling.md). |
| **리뷰에서 한 줄로** | API 는 접수만 하고 즉시 돌려주고, 발송은 워커가 큐에서 꺼내 비동기로 한다. |
