...
| | |
|---|---|
| **왜 필요했나** | 발송을 선점한 워커가 도중에 죽거나, 재시도를 다 쓴 잡이 DLQ 로 빠지면 캠페인이 영원히 "발송 중"에 멈췄다. 잘못 보낸 캠페인을 멈출 방법도 없었다. |
| **무엇을 썼나** | **DLQ 리스너**(실패 확정 + 완료 판정 + 알림), 60초 주기 **복구 스위퍼**(10분 넘게 멈춘 작업 재발행, 팬아웃은 이어서 재개), claim 토큰으로 종료 기록 보호, SMTP 타임아웃, **발송 중 중단**. |
| **결과** | 멈춘 작업이 사람 손 없이 풀리고, 늦게 끝난 워커가 남의 결과를 덮어쓰지 않는다. 복구·DLQ 건수는 지표(`mail_recovery_total`, `mail_dlq_received_total`)로 감시한다. |
| **리뷰에서 한 줄로** | 선점 이후 죽거나 실패해도 스스로 복구되고, 필요하면 발송을 즉시 멈출 수 있다. |
