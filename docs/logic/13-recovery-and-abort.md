...
<table>
<tr><th align="left">왜 필요했나</th><td>발송을 선점한 워커가 도중에 죽거나, 재시도를 다 쓴 잡이 DLQ 로 빠지면 캠페인이 영원히 "발송 중"에 멈췄다. 잘못 보낸 캠페인을 멈출 방법도 없었다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>DLQ 리스너</b>(실패 확정 + 완료 판정 + 알림), 60초 주기 <b>복구 스위퍼</b>(10분 넘게 멈춘 작업 재발행, 팬아웃은 이어서 재개), claim 토큰으로 종료 기록 보호, SMTP 타임아웃, <b>발송 중 중단</b>.</td></tr>
<tr><th align="left">결과</th><td>멈춘 작업이 사람 손 없이 풀리고, 늦게 끝난 워커가 남의 결과를 덮어쓰지 않는다. 복구·DLQ 건수는 지표(<code>mail_recovery_total</code>, <code>mail_dlq_received_total</code>)로 감시한다.</td></tr>
<tr><td colspan="2">선점 이후 죽거나 실패해도 스스로 복구되고, 필요하면 발송을 즉시 멈출 수 있다.</td></tr>
</table>
