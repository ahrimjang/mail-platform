...
| | |
|---|---|
| **왜 필요했나** | 부하 테스트는 한 번 재고 끝이다. 처리량·큐 적체·지연을 상시로 봐야 문제를 먼저 안다. |
| **무엇을 썼나** | **Micrometer**(계측은 infra 어댑터에서만) → **Prometheus**(수집) → **Grafana**(대시보드, 파일로 관리). RabbitMQ 큐 지표도 함께 수집. |
| **결과** | 처리량·큐 깊이·스로틀·SMTP p95·API·힙 6개 패널을 상시로 본다. 대시보드가 파일이라 변경 이력이 git 에 남는다. 운영에서는 SSH 터널로 접속. |
| **리뷰에서 한 줄로** | 발송 파이프라인 상태를 늘 켜져 있는 계기판으로 본다. |
