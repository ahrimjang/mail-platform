...
<table>
<tr><th align="left">왜 필요했나</th><td>캠페인마다 본문을 새로 쓰기 번거롭고, 수신자마다 이름 같은 값을 넣고 싶다.</td></tr>
<tr><th align="left">무엇을 썼나</th><td><b>템플릿 + <code>{{변수}}</code> 치환</b>. 캠페인 등록 시점에 내용을 복사(스냅샷)한다. 블록·텍스트·HTML 에디터 3종을 외부 라이브러리 없이 구현하고, 에디터 상태는 본문 안 주석에 저장한다. 템플릿(재사용 자산)과 이메일(이번 발송분)을 분리(V27).</td></tr>
<tr><th align="left">결과</th><td>템플릿을 고쳐도 이미 등록된 캠페인 내용은 바뀌지 않는다. 기본 템플릿은 복사해서 고치고, 언제든 원본으로 되돌릴 수 있다.</td></tr>
<tr><td colspan="2">템플릿으로 재사용하고, 보내는 순간 수신자별 값으로 채운다.</td></tr>
</table>
