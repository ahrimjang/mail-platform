...
| | |
|---|---|
| **왜 필요했나** | 캠페인마다 본문을 새로 쓰기 번거롭고, 수신자마다 이름 같은 값을 넣고 싶다. |
| **무엇을 썼나** | **템플릿 + `{{변수}}` 치환**. 캠페인 등록 시점에 내용을 복사(스냅샷)한다. 블록·텍스트·HTML 에디터 3종을 외부 라이브러리 없이 구현하고, 에디터 상태는 본문 안 주석에 저장한다. 템플릿(재사용 자산)과 이메일(이번 발송분)을 분리(V27). |
| **결과** | 템플릿을 고쳐도 이미 등록된 캠페인 내용은 바뀌지 않는다. 기본 템플릿은 복사해서 고치고, 언제든 원본으로 되돌릴 수 있다. |
| **리뷰에서 한 줄로** | 템플릿으로 재사용하고, 보내는 순간 수신자별 값으로 채운다. |
