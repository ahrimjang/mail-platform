...
### 09-21 ~ 09-23 · [워크일지](worklog/2026-09-23.md)
- **팀장 리뷰(09-22)** 와 그 자리에서 나온 방향 — 사내 알림 SaaS(SMS·카카오·웹푸시)를 이 저장소 포크로 시작한다. 계획은 [notification-fork-design.md](notification-fork-design.md).
- **포크 전 파이프라인 정리**: 포크 시점에 남은 결함은 두 저장소에서 각각 고쳐야 하므로 🔴 을 앞으로 당겼다.
- 클릭 추적 **서명키 롤오버**(현재 키로 서명, 현재+과거 키로 검증) — 없으면 키를 못 바꾼다.
- 수신거부 푸터를 `</body>` **앞**에 삽입(문서 밖에 놓여 버려지던 문제), 워커 종료 대기(SMTP 70초 < 종료 예산)로 **중복 발송 경로 차단**.
- **스팸 신고율을 바운스와 분리**해 별도 정지 축(시도 200 + 신고 3건 + 0.3%) — SES 정지선 0.5% 아래에서 먼저 멈춘다.
- **이메일 본문 검증**: `<script>`·`<iframe>`·`<form>` 거부, 미지원 변수·`{{고객명}}` 류 표기·102KB 초과는 경고.
- **계정 이메일 정규화(V37)** — `User@x.com` 으로 가입하면 `user@x.com` 으로 로그인이 안 되던 버그. 로그인 잠금 키도 같이 정규화(철자를 바꿔 잠금을 우회할 수 있었다).
