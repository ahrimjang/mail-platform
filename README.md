...
- **멀티테넌트 SaaS** — 가입=워크스페이스(회사) 등록, 관리자/운영자 역할, 루트 엔티티 워크스페이스 격리(남의 데이터는 404). 발송 인프라(SMTP/SES·저장소)는 플랫폼이 제공하고 **요금은 월 발송량 기준으로 청구**(관리 페이지 사용량·요금 카드 = 청구 기준 미터링) + **테넌트별 발송 속도 제한**(토큰버킷 — 한도 초과 메일은 1초 파킹 큐로 우회, 다른 테넌트는 무영향)
