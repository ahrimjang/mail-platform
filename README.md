...
- **멀티테넌트 SaaS** — 가입=워크스페이스(회사) 등록, 관리자/운영자 역할, 루트 엔티티 워크스페이스 격리(남의 데이터는 404), 관리 페이지에서 **BYO 커넥터 선택**(SMTP/저장소를 회사 계정에 연결해 인프라 비용이 회사에 직접 청구되는 구조 — 연동은 로드맵) + **월 발송량 미터링 카드**
